...
const bytes=required.filter(file=>!['assets/intro/opening.mp4','assets/main-menu-v2.png',...required.filter(file=>file.startsWith('assets/menu-buttons/'))].includes(file)).reduce((sum,file)=>sum+fs.statSync(path.join(root,file)).size,0);
