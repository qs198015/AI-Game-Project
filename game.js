(() => {
  ...
  const AI = [
    {id:'fang',name:'铁牙',emoji:'🐶',art:'assets/characters/portraits/ironfang_bulldog.png',portrait:'assets/characters/portraits/ironfang_bulldog.png',color:'#a77745',title:'直觉派保镖',hp:2,lie:0.38,doubt:0.55,
    ...
    {id:'crow',name:'鸮策',emoji:'🦉',title:'黑月酒馆老板',hp:2,lie:0.64,doubt:0.72,art:'assets/characters/portraits/owl_gentleman.png',portrait:'assets/characters/portraits/owl_gentleman.png',color:'#d1aa61',
  ...
  function reward(amount){const gemBoost=state.currentCards?.some(c=>c.id==='gem')?2:1,gained=amount*(state.activeEvent?.coin||1)*gemBoost;state.coins+=gained;floatingFeedback(els.coinsLabel,`+${gained}`,'coin');}
  ...
  function healFromHoney(){if(state.honeyUsed||!state.hand.some(c=>c.id==='honey')||state.playerHp>=4)return;state.honeyUsed=true;state.playerHp++;floatingFeedback(els.battleScreen.querySelector('.player-strip'),'+1 心','heal');log('🍯 午夜蜂蜜生效：判断精准，恢复 1❤️。');toast('午夜蜂蜜恢复生命！');}
  function hurtPlayer(reason){if(state.player.id==='shadow'&&!state.dodged&&Math.random()<.25){state.dodged=true;state.playerMood='smug';log('🐾 猫的直觉发动：夜影贴着伤害滑走了！');toast('闪避！');return false;}state.playerHp--;state.streak=0;state.playerMood='hurt';floatingFeedback(els.battleScreen.querySelector('.player-strip'),'−1 心','bad');log(reason);return true;}
  ...
  playIntro();
  window.__BLACK_MOON__={CARDS,AI,PLAYERS,EVENTS,ACHIEVEMENTS,MENU_BUTTON_ASSETS,claimTrue,freshState,endIntro};
})();
