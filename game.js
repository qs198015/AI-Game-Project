(() => {
  ...
  const CHARACTER_EMOTIONS={shadow:{idle:'confident',play:'confident',bluff:'triumphant',suspect:'suspicious',lose:'dejected',fail:'shocked',win:'triumphant'},tail:{idle:'sly',play:'sly',bluff:'sly',suspect:'nervous',lose:'embarrassed',fail:'embarrassed',win:'sly'},fang:{idle:'confident',play:'angry',bluff:'mocking',suspect:'angry',lose:'defiant',fail:'defiant',win:'mocking'},snow:{idle:'cute',play:'nervous',bluff:'scared',suspect:'scared',lose:'scared',fail:'scared',win:'lucky'},crow:{idle:'thinking',play:'thinking',bluff:'insight',suspect:'insight',lose:'surprised',fail:'surprised',win:'insight'}};
  const FAILURE_LINES={tail:'我只是故意输给你的。',fang:'下一次我一定相信你……才怪。',snow:'这一定是骰子的问题。',crow:'根据计算，这次失败概率为100%。',shadow:'刚才那局不算，我的胡须挡住牌了。'};
  const REACTION_LINES={shadow:{play:'看我的。',bluff:'别眨眼。',suspect:'有意思。',win:'上钩了。',fail:'失手而已。'},tail:{play:'当然是真的。',bluff:'当然是真的。',suspect:'你确定？',win:'真乖。',fail:'故意的。'},fang:{play:'看好了。',bluff:'敢不信？',suspect:'我才不信。',win:'哈！',fail:'不算！'},snow:{play:'给、给你。',bluff:'别看我……',suspect:'等等……',win:'好耶！',fail:'骰子问题！'},crow:{play:'请判断。',bluff:'概率正常。',suspect:'概率不对。',win:'如我所料。',fail:'计算失误。'}};
  ...
  function cueReaction(character,event){
    const line=REACTION_LINES[character.id]?.[event];if(!line)return;
    if(character===state.player){state.playerReaction=event;state.playerQuip=line;}else{character.reaction=event;character.quip=line;}
  ...
  function playSfx(name){
    if(!(state?.sound??save.sound))return;
    const preset={button_click:[240,.055,.1,'triangle'],card_hover:[420,.018,.055,'sine'],card_flip:[690,.04,.12,'triangle'],card_play:[330,.045,.15,'triangle'],trust:[540,.05,.2,'sine'],doubt:[125,.055,.18,'sawtooth']}[name];
    if(!preset)return;
  ...
  function say(name,text){
    ...
    const speaker=state?.rivals?.[speakerIndex];
  ...
  function playPenaltyAnimation(id,result){
    ...
    delay(()=>{closeDialog(els.penaltyDialog);document.body.classList.remove('penalty-vignette');els.penaltyStage.className='penalty-stage';render();continueRound();},1900);
  }
  ...
  function advance(){if(state.pendingPenalty)return showPenalty();continueRound();}
  ...
  playIntro();
  window.__BLACK_MOON__={CARDS,AI,PLAYERS,EVENTS,ACHIEVEMENTS,MENU_BUTTON_ASSETS,claimTrue,freshState,endIntro,debug:{getState:()=>state,startBoss,beginRound,advance,resolvePenalty}};
})();
