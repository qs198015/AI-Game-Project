(() => {
  ...
  function startBoss(){const boss=AI.find(ai=>ai.id==='crow');state.boss=true;state.round=state.maxRounds+1;state.rivals=[{...boss,hp:4,maxHp:4,doubt:.8,lie:.72,mood:'thinking',hand:Array.from({length:5},()=>rand(CARDS)),title:'终局 Boss · 黑月老板'}];state.active=0;state.activeEvent={id:'boss',emoji:'🌑',name:'老板亲自下场',text:'鸦策每次判断都更敏锐；击败他才能带走奖赏。',suspicion:.08};tableEffect('BOSS 入 席','bad');log('🌑 八局钟声敲响。鸦策锁上酒馆大门，亲自坐到了牌桌前。');beginRound();}
