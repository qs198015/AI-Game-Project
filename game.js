(() => {
  ...
  function dialogueDuration(text){
    ...
    return Math.min(12000,Math.max(4500,2500+(textLength/4)*1000));
  }
  function say(name,text){
    ...
    b.textContent=name;s.textContent=text;els.enemySpeech.append(b,s);[...els.opponents.children].forEach((node,index)=>{const dialogue=node.querySelector?.('.opponent-dialogue');if(!dialogue)return;dialogue.classList.toggle('is-visible',index===speakerIndex);dialogue.setAttribute('aria-hidden',String(index!==speakerIndex));if(index===speakerIndex)dialogue.textContent=text;});const ironfangDialogue=els.ironfangHud.querySelector?.('.opponent-dialogue');if(ironfangDialogue){const isIronfang=speaker?.id==='fang';ironfangDialogue.classList.toggle('is-visible',isIronfang);ironfangDialogue.setAttribute('aria-hidden',String(!isIronfang));if(isIronfang)ironfangDialogue.textContent=text;}clearTimeout(speechTimer);speechTimer=setTimeout(()=>{if(speaker){speaker.isSpeaking=false;render();}},dialogueDuration(text));
  ...
  function opponentInfoMarkup(ai,status,trust){return `<strong class="opponent-name">${ai.name}</strong><q class="opponent-dialogue${ai.isSpeaking?' is-visible':''}" aria-hidden="${!ai.isSpeaking}">${ai.isSpeaking?ai.lastSpeech:''}</q><span class="opponent-hearts hp" aria-label="${Math.max(0,ai.hp)} 点生命">${heartMarkup(Math.max(0,ai.hp),ai.maxHp||3)}</span><small class="opponent-trust">可信度：${trust}%</small><span class="pet-status" aria-label="当前状态：${status}">${status}</span>`;}
  function renderOpponents(){els.opponents.innerHTML='';let ironfangInfo='';state.rivals.forEach((ai,i)=>{const div=document.createElement('div');div.className=`opponent character-${ai.id} mood-${ai.mood}${ai.reaction?` reaction-${ai.reaction}`:''}${i===state.active&&ai.hp>0?' active':''}${ai.hp<=0?' out':''}`;div.style.setProperty('--hero-color',ai.color||'#a47b56');const status=ai.hp<=0?'离桌':i===state.active?EXPRESSIONS[ai.mood]:'旁观',trust=clamp(ai.trust??Math.round((1-ai.doubt)*100),0,100),info=opponentInfoMarkup(ai,status,trust);div.innerHTML=`<span class="avatar">${portraitMarkup(ai,ai.mood)}${ai.quip?`<q class="reaction-bubble">${ai.quip}</q>`:''}</span><span class="ai-hand-backs" aria-label="${ai.name}有${ai.hand?.length||0}张隐藏手牌">${(ai.hand||[]).slice(0,5).map(()=>'<i></i>').join('')}</span><div class="opponent-info">${info}</div>`;if(ai.id==='fang')ironfangInfo=info;els.opponents.append(div);});els.ironfangHud.innerHTML=ironfangInfo?`<div class="opponent-info">${ironfangInfo}</div>`:'';els.ironfangHud.classList.toggle('hidden',!ironfangInfo);}
  ...
  playIntro();
  window.__BLACK_MOON__={CARDS,AI,PLAYERS,EVENTS,ACHIEVEMENTS,MENU_BUTTON_ASSETS,claimTrue,createRivals,freshState,dialogueDuration,endIntro,debug:{getState:()=>state,startBoss,beginRound,startAISequence,advanceAISequence,advance,resolvePenalty,cueReaction,cameraEffect,aiChallengeChance}};
})();
