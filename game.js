(() => {
  ...
  function startMenuPetMotion(){
    if(save.reducedMotion||window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)return;
    const pets=[...document.querySelectorAll?.('#menuPetEffects .pet-motion')||[]];
    const later=(min,max,task)=>setTimeout(task,min+Math.random()*(max-min));
    pets.forEach(pet=>{
      const blink=()=>later(4000,9000,()=>{if(!els.menuScreen.classList.contains?.('hidden')){pet.classList.add('is-blinking');const duration=120+Math.random()*60;setTimeout(()=>pet.classList.remove('is-blinking'),duration);if(Math.random()<.16)setTimeout(()=>{pet.classList.add('is-blinking');setTimeout(()=>pet.classList.remove('is-blinking'),duration);},duration+100);}blink();});
      blink();
      if(pet.matches?.('.pet-fox,.pet-rabbit')){const twitch=()=>later(6000,12000,()=>{if(!els.menuScreen.classList.contains?.('hidden')){pet.classList.add('is-twitching');setTimeout(()=>pet.classList.remove('is-twitching'),450);}twitch();});twitch();}
    });
  }
