(() => {
  ...
  function renderOpponents(){els.opponents.innerHTML='';state.rivals.forEach((ai,i)=>{const div=document.createElement('div');div.className=`opponent seat-slot seat-slot-${i+1} character-${ai.id} mood-${ai.mood}${ai.reaction?` reaction-${ai.reaction}`:''}${i===state.active&&ai.hp>0?' active':''}${ai.hp<=0?' out':''}`;div.dataset.seat=String(i+1);div.style.setProperty('--hero-color',ai.color||'#a47b56');const status=ai.hp<=0?'离桌':i===state.active?EXPRESSIONS[ai.mood]:'旁观',trust=clamp(ai.trust??Math.round((1-ai.doubt)*100),0,100);div.innerHTML=`<i class="seat-contact" aria-hidden="true"></i><span class="avatar seat-actor">${portraitMarkup(ai,ai.mood)}${ai.quip?`<q class="reaction-bubble">${ai.quip}</q>`:''}</span><div><strong>${ai.name}</strong><small>${ai.title}</small><span class="pet-status">${status}</span><span class="hp" aria-label="${Math.max(0,ai.hp)} 点生命">${heartMarkup(Math.max(0,ai.hp),ai.maxHp||3)}</span><span class="affinity"><i style="width:${trust}%"></i></span><small class="affinity-label">信任 ${trust}%</small></div>`;els.opponents.append(div);});}
