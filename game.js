(() => {
  ...
  const menuButtons=[els.startBtn,els.rulesBtn,els.storyModeBtn,els.trialBtn,els.charactersBtn,els.settingsBtn];
  const setMenuHover=active=>menuButtons.forEach(button=>button.classList.toggle('is-hovered',button===active));
  menuButtons.forEach(button=>{button.addEventListener('pointerenter',()=>setMenuHover(button));button.addEventListener('pointerleave',()=>setMenuHover(null));button.addEventListener('focus',()=>setMenuHover(button));button.addEventListener('blur',()=>setMenuHover(null));});
  els.menuScreen.addEventListener('click',event=>{if(event.target.closest?.('button')){setMenuHover(null);startMusic();tone();}});
