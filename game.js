(() => {
  ...
  function reveal(card){els.centerCard.classList.remove('flip');delete els.centerCard.dataset.card;els.centerCard.innerHTML='<span>☾</span>';requestAnimationFrame(()=>{els.centerCard.dataset.card=card.id;els.centerCard.classList.add('flip');els.centerCard.innerHTML=`<span>${card.emoji}</span>`;});}
