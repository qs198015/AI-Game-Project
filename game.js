(() => {
  ...
  els.menuScreen.addEventListener('keydown',event=>{if((event.key==='Enter'||event.key===' ')&&event.target.matches?.('.menu-button-image')){event.preventDefault();event.target.click();}});
